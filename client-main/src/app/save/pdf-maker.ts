...
export class PdfMaker {
  ...
  // Color palette
  palette = ['#fdf4bd', '#ffe6f1', '#ffb99e', '#c2def3', '#c5bbe0',
  '#f0f1f5', '#d2eddc', '#b3f9ef', '#fcdfd7', '#f3add1', '#c5fdb6'];
  colorForName = [];
  ...
  generatePdf(classes: Array<Class>, id: string){
    ...
    pdfDocGenerator.getBlob((blob) => {
      ...
      if (window.navigator && window.navigator.msSaveOrOpenBlob) {
        window.navigator.msSaveOrOpenBlob(blob, 'Ders Programı (ituprogram.com).pdf');  
      }
      else {
      ...
      content: [
        ...
        { text: 'Kaldığın yerden devam etmek istersen:', style: 'subHeader' },
        {
          ...
          table: {
            widths:['auto', 'auto'],
            body: [
              [{text: 'PIN numaran:', bold: true, margin: [15,0,0,0]}, this.id],
              [{text: 'Link:', bold:true, margin: [15,0,0,0] }, {text: 'http://ituprogram.com/id/' + this.id}]
            ]
          }
        }
      ],
 ...
 generataWeeklyTable() {
    ...
    for (let j = 1; j < 6; j++) {
      ...
      weekTableArray.forEach(slot => {
        ...
        if(slot.day == j && slot.hour == i) {
          ...
          row.push({text: this.shorthenLongWords(slot.name), fontSize: 11, fillColor: this.palette[index]});
          emptyCell = false;
        }
      });
  ...
  weeklyTableLayout = {
    ...
    paddingLeft: () => { return 5; },
    paddingRight: () => { return 5; },
    hLineWidth: () => { return 0.2; },
    vLineWidth: () => { return 0.2; },
    hLineColor: () => {return '#999999';},
    vLineColor: () => {return '#999999';},
  };
